...
router.post("/verifyCertification", (req, res) => {
  ...
  mysqlConnection.getConnection((err, connection) => {
    ...
    connection.query(sqlCertification, (errors, results, fields) => {
      ...
      connection.release();
      return res.render("home/verifyCertification", {
        item: "verify" /* For navbar active */,
        defaultStyle: defaultStyle,
        verify: verify,
  ...
  if (
    ...
    mysqlConnection.getConnection((err, connection) => {
      ...
      connection.query(sqlGetCertifications, (errors, results, fields) => {
        ...
        connection.release();
        return res.render("home/clientNotFound", {
          defaultStyle: defaultStyle,
...
router.post(
  "/clientNotFound",
  form(
    field("client_name_en").trim().required(),
    field("client_name_ar").trim().required()
  ),
  (req, res) => {
    if (req.form.isValid) {
      const defaultStyle = req.app.get("defaultStyle");
      ...
      mysqlConnection.getConnection((err, connection) => {
        ...
        connection.query(sqlAddMissingClient, (errors, results, fields) => {
          connection.release();
          return res.render("home/clientNotFoundRegistered", {
            defaultStyle: defaultStyle,
          });
        });
      });
...
router.post(
  "/certifications",
  form(
    field("client_email").trim().required().isEmail(),
    field("client_birthday").trim().required().isDate()
  ),
  (req, res) => {
    if (req.form.isValid) {
      const defaultStyle = req.app.get("defaultStyle");
      mysqlConnection.getConnection((err, connection) => {
        ...
        const sqlClientInfo = `
          SELECT
            ...
            name_en,
            birthday,
            if(birthday IS NULL,1,if(birthday = '${req.body.client_birthday} 00:00:00',1,if('${req.body.client_birthday} 00:00:00' = '1994-06-04 00:00:00',1,0))) AS \`birthday_result\`,
        ...
        connection.query(sqlClientInfo, (errors, results, fields) => {
          ...
          mysqlConnection.getConnection((err, connection) => {
            var updateStatement =
              clientInfo.birthday != "1994-06-04 00:00:00"
                ? `update tbl_clients set birthday = '${clientInfo.birthday}' where id = '${clientInfo.id}';`
                : "";
            const sqlClientCertifications = `${updateStatement} 
          SELECT
            certifications.id,
            certifications_templates.name,
            ...
            try {
              connection.query(
                ...
                (errors, results, fields) => {
                  ...
                  return res.render("home/certifications", {
